...
class ClinicProfileAPI(generics.GenericAPIView):
    serializer_class = ClinicProfileSerializer

    def post(self, request, *args, **kwargs):

        try:
            ProfileEmail = apps.get_model("partners", "Clinic")
            clinicid = request.data.get("ClinicId")
            emailData = ProfileEmail.objects.get(id=clinicid)
            ...
            response.Result = ClinicProfileSerializer(user).data
            response.Status = 200
